...
if not project_endpoint or not model_deployment:
    raise ValueError("PROJECT_ENDPOINT and MODEL_DEPLOYMENT must be set in .env file")
...
async def main():
    ...
    outputs: list[AgentResponse] = result.get_outputs()
    
    
    # Display outputs
    if outputs:
        for i, msg in enumerate(outputs[-1].messages, start=1):
